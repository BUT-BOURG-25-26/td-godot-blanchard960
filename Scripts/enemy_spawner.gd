...
func _on_timer_timeout():
	...
	if( -5 > (player.global_position.x-x)+(player.global_position.z-z) || (player.global_position.x-x)+(player.global_position.z-z) > 5) :
		# Spawner votre enemy
		...
		var enemy = enemy_scene.instantiate()
		get_parent().add_child(enemy)
		enemy.position = player.global_position +Vector3(x+5,0.0,z-5)
